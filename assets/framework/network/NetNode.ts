...
export class NetNode {
    ...
    public init(socket: ISocket, protocol: IProtocolHelper, networkTips: INetworkTips = null, execFunc: ExecuterFunc = null) {
        log.printDebug(`NetNode init socket`);
        this._socket = socket;
    ...
    protected onConnected(event: Event) {
        log.printDebug("NetNode onConnected!")
    ...
    protected onMessage(msg: NetData): void {
        ...
        if (!this._protocolHelper.checkPackage(msg)) {
            log.printError(`NetNode checkHead Error`);
            return;
        }
        // 接受到数据，重新定时收数据计时器
    ...
    public send(buf: NetData): boolean {
        if (this._state == NetNodeState.Working) {
            ...
            log.printDebug("NetNode socket is busy! current state is " + this._state);
            return false;
        } else {
            log.printError("NetNode request error! current state is " + this._state);
            return false;
        }
    }
    /********************** 回调相关处理 *********************/
    public setResponeHandler(cmd: number, callback: NetCallFunc, target?: any): boolean {
        if (callback == null) {
            log.printError(`NetNode setResponeHandler error ${cmd}`);
            return false;
        }
    ...
    public addResponeHandler(cmd: number, callback: NetCallFunc, target?: any): boolean {
        if (callback == null) {
            log.printError(`NetNode addResponeHandler error ${cmd}`);
            return false;
        }
    ...
    protected resetReceiveMsgTimer() {
        ...
        this._receiveMsgTimer = setTimeout(() => {
            log.printWarn("NetNode recvieMsgTimer close socket!");
            this._socket.close();
        }, this._receiveTime);
    }
