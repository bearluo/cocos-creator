...
import { Node as ccNode, math, sys } from 'cc';
import { LoginMainInetMsg } from "../network/MainInetMsg";
import { CenterInet } from "./center";

export class LoginInet extends LoginMainInetMsg<typeof proto.login_server.GS_LOGIN_MSG> {
    cmd = proto.login_server.GS_LOGIN_MSG;
}

export class LoginCenter extends CenterInet<LoginInet> {
    // 登录信息
    ...
    initEvents() {
        this.inet.initMainID(GS_LOGIN_MSGID.GS_LOGIN_MSGID_LOGIN);
        app.event.bindEvent({
    ...
    initRegister(inet:LoginInet,cmd:typeof proto.login_server.GS_LOGIN_MSG) {
        inet.bindMsgStructPB(cmd.LOGIN_MSGID_LOGINERROR, proto.login_server.login_error_s)
        inet.bindRecvFunc(cmd.LOGIN_MSGID_LOGINERROR, this.OnRecv_LoginFail.bind(this))

        inet.bindMsgStructPB(cmd.LOGIN_MSGID_LOGINEND, proto.login_server.login_end_s)
        inet.bindRecvFunc(cmd.LOGIN_MSGID_LOGINEND, this.OnRecv_LoginEnd.bind(this))

        inet.bindMsgStructPB(cmd.LOGIN_MSGID_TIPS, proto.login_server.login_tips_s)
        inet.bindRecvFunc(cmd.LOGIN_MSGID_TIPS, this.OnRecv_Tips.bind(this))

        inet.bindMsgStructPB(cmd.LOGIN_MSGID_LOGINNEW, proto.login_server.login_new_c)
    }
    ...
    // Web Guset 登录
    loginGuestWeb(name, onlyoneKey, password, is_reg) {
        this.m_eLoginType = LOGINTYPE.WEB_GUEST
        this.m_strLoginAccount = name
        this.m_strLoginMd5Password = password
        this.m_strLoginMachineName = name
        this.m_strLoginBuffHD = onlyoneKey
    ...
    // 申请web游客账号
    applyWebGuestAccount() {
        return new Promise((resolve: (value: WebGuastData) => void, reject: (reason?: Error) => void) => {
            //请求登录
            let url = httpConfig.path_pay + "Web/createUserAccount"
            let params: any = {
                ...
                if (bSuccess) {
                    ...
                    if (data.status == 1) {
                        app.file.writeStringToFile({
                            ...
                            filePath: center.login.getLoginWebGuestDataPath(),
                            bEncrypt: true,
                        });
                        resolve(data.data);
                    } else if (data.info) {
                        ...
                        reject(new Error(fw.language.get("data error, please login again")))
                    }
                } else {
                    reject(new Error(fw.language.get("Something went wrong with login, please login again")))
                }
            }
            app.http.post({
                //请求链接
    ...
    // web 游客账号登录
    visitorLoginWeb(account: string, pwd: string, guid: string) {
        //请求登录
        let url = httpConfig.path_pay + "Login/webguest"
        let extra = {} as any
        extra.isVpn = app.native.device.isVpnUsed()
        extra.pwd = pwd
        let params = this.getLoginPhpParams(account, extra, guid)
        let checkCallback = async (bSuccess, response) => {
            if (bSuccess) {
                let data = response
                if (data.status == 1) {
                    if (await center.login.isAllowLogin(data.wflag) && app.http.checkPhpSign(response)) {
                        ...
                        center.roomList.downloadRoomInfo(data.version)
                        center.login.loginGuestWeb(data.account, data.account, data.password, data.is_reg == 1)
                        this.m_pwflag = data.pwflag
                        this.m_vflag = data.vflag
                        this.m_shopMaxMamount = data.mamount
                    }
                } else if (data.info) {
                    app.popup.showToast({ text: data.info })
                } else {
    ...
    /**拉取leo数据 */
    doLeoJump() {
        if (center.user.webServer.leoPackageGuideData) {
            return;
        }
        const user_sub_type = app.native.device.getOperatorsSubID();
        if (user_sub_type.startsWith(`slm_30`)) {
            let params: any = {
                user_id: center.login.getLoginEndUserDBID(),
                user_sub_type: user_sub_type,
                timestamp: app.func.time(),
            }
            params.sign = app.http.getSign(params);
            app.http.post({
                url: `${httpConfig.path_pay}hall/offlineCheck`,
                params: params,
            });
        }
    }
    ...
    updateServerProxy(callback) {
        ...
        if (app.func.isBrowser()) {
            url = httpConfig.path_pay + (app.socket.socketData.url_login.startsWith(`https`) ? "Server/wssProxy" : "Server/Proxy");
        } else {
            url = httpConfig.path_pay + "Server/Proxy";
        }
    ...
    //返回本地保存数据路径
    getLoginWebGuestDataPath() {
        return PATHS.LoginWebGuestData + `/` + app.native.device.getOperatorsSubID();
    }
}


export interface WebGuastData {
    account: string,
    loginpwd: string,
    guid: string,
    ctime: number,
