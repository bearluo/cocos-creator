...
export class BindInetMsg<T=any> extends EventTarget {
    ...
    getStructName(data: GetStructNameParam): string {
        ...
        if (data.name != null) {
            ...
            log.printError(`bindMessage error`);
        }
        return structName;
    }
    /**“注册回调”“注册结构”等 */
    ...
    /**发送消息 */
    sendMessage<T>(data: SendMessageParam<T>): boolean {
        let {cmd,data:sdata = {}} = data;
        return this.sendData(cmd, sdata);
    }
    /**绑定结构体 */
    ...
    /**解绑结构体，带继承 */
    unregisterStruct(structName: string | number) {
        if (typeof structName == `number`) {
            [
                { cmd: structName },
                { cmd: structName, callback: () => { } },
            ].forEach(element => {
                this.structParse.unregisterStruct(this.getStructName(element));
            });
        } else {
    ...
    OnRecvData(pByteStream: ByteStream) {
        ...
        if (strBindStructName == null) {
            // log.printError(`${this.constructor.name}:OnRecvDataRoot(): warning RootID:${this.rootInetMsg.nRootID} MainID:${this.nMainID} SubID:${nSubID} didn't bind struct!!!`)
            return
        }
        let tblData = null;
        try {
            ...
            if (tblData == null) {
                log.printError(`${this.constructor.name}:OnRecvDataRoot(): warning SubID:${nSubID} StructName:${strBindStructName} !!!`);
            }
        } catch (e) {
            log.printWarn(strBindStructName, e);
        }
        let bindStructPB = this.mapStructPB.get(nSubID);
        if (bindStructPB != null) {
            let pbData: any;
            try {
                pbData = bindStructPB.decode(tblData.szBuff, tblData.nBuffLen);
            } catch (e) {
                log.printError(e);
            }
    ...
    exeMsg(nSubID: number, dict: any, pByteStream: ByteStream) {
        if (this.mapCallback.has(nSubID)) {
            ...
            if (data.printLog) {
                log.print(data.funcName || data.callback.name, dict)
            }
            if (data.bQueue) {
                dict.__nQueueTime = Date.now();
    ...
    triggerMessageList(message?: MessageParam) {
        ...
        if (this.bCanTriggerMessage) {
            if (this.messageList.length > 0) {
                ...
                //测试输出
                this.bPrintMessageFuncName && log.print(`do message: ${next.data.funcName ?? `null`}`);
                //接收和执行的时差
                next.dict.__nDiffTime = (Date.now() - next.dict.__nQueueTime) / 1000;
                //执行
    ...
    /**发送消息 */
    sendData(nSubID: number, dict?: any) {
        log.print("sendData",nSubID , dict);
        return this.rootInetMsg.sendData(this.getSendData(nSubID, dict));
    }
    /**获取发送消息数据 */
    getSendData(nSubID: number, dict?: any) {
        let strBindStructPB = this.mapStructPB.get(nSubID);
        if (strBindStructPB != null) {
            let szBuff = strBindStructPB.encode(dict).finish();
            dict = {
                nBuffLen: szBuff.length,
                szBuff: szBuff,
            }
            return this.rootInetMsg.getSendData(this.nMainID, (pSendStream: ByteStream) => {
                this.package(pSendStream, nSubID, "SendProtobufMsg", dict)
            });
        } else {
            let bindStructName = this.mapStruct.get(nSubID);
            assert(bindStructName,"bindStructName is null")
            return this.rootInetMsg.getSendData(this.nMainID, (pSendStream: ByteStream) => {
                this.package(pSendStream, nSubID, bindStructName, dict)
            });
        }
    }
    /**打包消息数据 */
